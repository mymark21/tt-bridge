...
async function connect(trigger: 'startup' | 'popup' | 'start' | 'reconnect'): Promise<void> {
  ...
  socket.onopen = () => {
    ...
    console.log('[tt-bridge] Connected to daemon');
  ...
  socket.onmessage = async (event) => {
    try {
      ...
      console.error('[tt-bridge] Message handling error:', error);
    }
  };
  ...
  socket.onclose = () => {
    ...
    console.log('[tt-bridge] Disconnected from daemon');
    ws = null;
    socketPort = null;
...
function resetWindowIdleTimer(workspace: string): void {
  ...
  session.idleTimer = setTimeout(async () => {
    ...
    try {
      ...
      console.log(`[tt-bridge] Automation window ${current.windowId} (${workspace}) closed (idle timeout)`);
    } catch {
      // Already gone.
    }
...
async function getAutomationWindow(workspace: string): Promise<number> {
  ...
  console.log(`[tt-bridge] Created incognito automation window ${session.windowId} (${workspace})`);
...
chrome.windows.onRemoved.addListener((windowId) => {
  for (const [workspace, session] of automationSessions.entries()) {
    if (session.windowId === windowId) {
      console.log(`[tt-bridge] Automation window closed (${workspace})`);
      if (session.idleTimer) clearTimeout(session.idleTimer);
      automationSessions.delete(workspace);
    }
  }
...
async function initialize(): Promise<void> {
  ...
  console.log('[tt-bridge] Extension initialized');
}
...
async function resolveTabId(tabId: number | undefined, workspace: string): Promise<number> {
  if (workspace === 'human') {
    ...
    const target = tabs.find((tab) => tab.id && isDebuggableUrl(tab.url));
    if (target?.id) return target.id;
    throw new Error('No active debuggable tab in the human window');
  }
  ...
  if (tabId !== undefined) {
    try {
      ...
      console.warn(`[tt-bridge] Tab ${tabId} URL is not debuggable (${tab.url}), re-resolving`);
    } catch {
      console.warn(`[tt-bridge] Tab ${tabId} no longer exists, re-resolving`);
    }
  }
  ...
  if (reuseTab?.id) {
    ...
    try {
      ...
      console.warn(`[tt-bridge] data: URI was intercepted (${updated.url}), creating fresh tab`);
    } catch {
...
async function handleNavigate(cmd: Command, workspace: string): Promise<Result> {
  ...
  await new Promise<void>((resolve) => {
    ...
    setTimeout(() => {
      chrome.tabs.onUpdated.removeListener(listener);
      timedOut = true;
      console.warn(`[tt-bridge] Navigate to ${targetUrl} timed out after 15s`);
      resolve();
    }, 15000);
  });
